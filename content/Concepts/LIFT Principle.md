---
{"publish":true,"permalink":"/Concepts/LIFT Principle.md","created":"2025-09-27T13:55:59.172+02:00","modified":"2025-09-27T18:09:01.849+02:00","published":"2025-09-27T18:09:01.849+02:00","cssclasses":""}
---


LIFT is an organization principle for software developers.  It was introduced by Google as part of the [AngularJS](https://github.com/angular/angular.js?ref=dsebastien.net) and [Angular](https://angular.io/?ref=dsebastien.net) style guides. LIFT can be applied to code, Personal Knowledge Management (PKM) and Information Management (IM) in general.

LIFT is an acronym for:
- **L**ocate
- **I**dentify
- **F**lat structure as long as we can
- **T**ry to stay dry

The goal is to keep files and folder structures as flat as possible to quickly locate and identify content. It recommends applying the "Don't Repeat Yourself" (DRY) principle to avoid duplicating information as much as possible.
